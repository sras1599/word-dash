...
Storybook. It renders the production `GameBoard` and uses the production `gameReducer`,
but it does not create a room or import the HTTP and WebSocket clients.
...
- `GameSimulationHarness.tsx` presents the controls, timer, game board, and event log.
